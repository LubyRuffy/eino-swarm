...
describe("copyText", () => {
  ...
  // The desktop window is WKWebView. Clipboard write is often denied there;
  // execCommand still runs inside the click. Doing that first is why the
  // copy button is not a no-op. The payload has to go on the copy event:
  // WebKit's boolean is true even when paste is empty.
