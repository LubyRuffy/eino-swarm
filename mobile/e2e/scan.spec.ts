...
test("a saved ticket shows host tabs and connecting, not the scan form", async ({ page }) => {
  ...
  // The corner is New chat; Add a PC is a menu row, not a second control.
  await expect(page.getByTestId("new-chat-top")).toBeVisible()
  await expect(page.getByRole("button", { name: "添加 PC" })).toHaveCount(0)
  ...
  await expect(failed).toContainText("连不上这台 PC")
  await expect(failed).not.toContainText("服务端拒绝")
  const pip = page.getByLabel("path=offline")
  await expect(pip).toBeVisible()
  await expect(pip).toHaveClass(/bg-muted-foreground/)
  await expect(pip).not.toHaveClass(/--online/)
  await expect(page.getByRole("heading", { name: "扫码绑定这台 PC" })).toHaveCount(0)
  await expect(page.getByRole("button", { name: "重试" })).toBeVisible()
  await page.getByRole("button", { name: "菜单" }).click()
  await page.getByRole("menuitem", { name: "添加 PC" }).click()
