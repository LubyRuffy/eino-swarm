...
describe("native phone apps", () => {
  ...
  it("asks the page before Android back finishes the activity", () => {
    ...
    const streaming = java.indexOf("registerPlugin(StreamBodyPlugin.class)")
    const bridge = java.indexOf("super.onCreate")
    expect(registered).toBeGreaterThan(-1)
    expect(streaming).toBeGreaterThan(registered)
    expect(bridge).toBeGreaterThan(streaming)
  ...
  it("streams a model completion instead of waiting for the whole POST", () => {
    // CapacitorHttp's data task returns the body in one piece. The completion
    // has its own reader so the transcript moves while the model is still talking.
    const swift = read("ios/App/App/StreamBodyPlugin.swift")
    expect(swift).toMatch(/URLSessionDataDelegate/)
    expect(swift).toMatch(/didReceive data: Data/)
    expect(swift).toMatch(/timeoutIntervalForRequest/)
    expect(swift).toMatch(/httpMethod = "POST"/)
    expect(swift).toMatch(/willPerformHTTPRedirection/)
    expect(swift).toMatch(/completionHandler\(nil\)/)
    const bridge = read("ios/App/App/BridgeViewController.swift")
    expect(bridge).toMatch(/registerPluginInstance\(StreamBodyPlugin\(\)\)/)
    const storyboard = read("ios/App/App/Base.lproj/Main.storyboard")
    expect(storyboard).toMatch(/customClass="BridgeViewController"/)
    expect(storyboard).toMatch(/customModule="App"/)
    const pbx = read("ios/App/App.xcodeproj/project.pbxproj")
    expect(pbx).toMatch(/StreamBodyPlugin\.swift in Sources/)
    expect(pbx).toMatch(/BridgeViewController\.swift in Sources/)
    const java = read("android/app/src/main/java/com/lubyruffy/zwai/StreamBodyPlugin.java")
    expect(java).toMatch(/HttpURLConnection/)
    expect(java).toMatch(/setReadTimeout/)
    expect(java).toMatch(/setRequestMethod\("POST"\)/)
    expect(java).toMatch(/setInstanceFollowRedirects\(false\)/)
    expect(java).toMatch(/notifyListeners\("chunk"/)
    expect(java).toMatch(/"https"\.equals\(protocol\)/)
  })
