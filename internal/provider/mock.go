...
func mockAnswer(task string, results []string) string {
	...
	b.WriteString(mockChartBlock(results))
	b.WriteString(mockCodeAndMathBlock())
	b.WriteString("\n> Generated by the offline scripted provider: no model was called.\n")
	return b.String()
}

// mockChartBlock is a chart fence whose values are the report lengths, so the
// offline provider exercises the renderer without baking in a sample domain.
func mockChartBlock(results []string) string {
	if len(results) < 2 {
		return ""
	}
...
// mockCodeAndMathBlock exercises the transcript highlighter and KaTeX on the
// same offline path as a real answer, without a sample domain in the body.
// Under `go test` it is empty: each extra word is another mockChunkDelay, and
// that was enough to blow the engine package's 10m budget. `zwai --mock` and
// Playwright still emit it (`testing.Testing` is false in `go run`).
func mockCodeAndMathBlock() string {
	if testing.Testing() {
		return ""
	}
	return mockCodeAndMathSource()
}
...
var (
	mockAskUserMu sync.Mutex
	mockAskUser   bool
)

// SetMockAskUser makes the scripted manager call ask_user before it spawns.
// Tests that need a blocked question call this with true and restore false
// in Cleanup. Default mock runs must not ask, or every suite hangs.
func SetMockAskUser(v bool) {
	mockAskUserMu.Lock()
	mockAskUser = v
	mockAskUserMu.Unlock()
}

func mockAskUserEnabled() bool {
	mockAskUserMu.Lock()
	defer mockAskUserMu.Unlock()
	return mockAskUser
}

func mockAskMessage(turn int) *schema.Message {
	args, _ := json.Marshal(map[string]any{
		"questions": []map[string]any{
			{
				...
				"options": []map[string]any{
					{"id": "safer", "label": "Prefer the safer path (Recommended)"},
	...
	return &schema.Message{
		Role:    schema.Assistant,
		Content: "A choice here would change the work, so I am asking.\n",
		ToolCalls: []schema.ToolCall{
			call(fmt.Sprintf("mock-ask-%d", turn), askUserToolName, string(args)),
		},
	}
}

func mockAskOrPlan(turn int, msgs []*schema.Message) *schema.Message {
	// Planning always asks first (explore → ask → propose). Default
	// non-plan runs must not, or every suite hangs on a card.
	if (mockAskUserEnabled() || mockPlanning(msgs)) && !alreadyCalledTool(msgs, askUserToolName) {
		return mockAskMessage(turn)
	}
	if mockPlanning(msgs) {
		if alreadyCalledTool(msgs, proposePlanToolName) {
			return schema.AssistantMessage("The plan is ready.\n", nil)
		}
		body := "# Plan\n\n## Overview\n\n" + lastUserText(msgs) + "\n\n## Steps\n\n1. Inspect current state.\n2. Make the change.\n3. Verify.\n"
		args, _ := json.Marshal(map[string]any{"markdown": body})
		return &schema.Message{
			Role:    schema.Assistant,
			Content: "Drafting the plan.\n",
			ToolCalls: []schema.ToolCall{
				call(fmt.Sprintf("mock-plan-%d", turn), proposePlanToolName, string(args)),
			},
		}
	}
	return nil
...
func mockPlanning(msgs []*schema.Message) bool {
	for _, m := range msgs {
		if m == nil || m.Role != schema.System {
			continue
		}
		if strings.Contains(m.Content, "You are planning") {
			return true
		}
	}
	return false
...
func alreadyCalledTool(msgs []*schema.Message, name string) bool {
	for _, m := range msgs {
		if m == nil {
			continue
		}
...
// SetMockFailTimes makes the next n scripted model calls return err, then
// succeeds. A truncated tool-call JSON that the host retries once is the
// case this exists for.
func SetMockFailTimes(n int, err error) {
	mockFailMu.Lock()
	if n < 0 {
		n = 0
	}
