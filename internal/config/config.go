...
type SwarmConfig struct {
	...
	// finished turn, and for an inbox label when a wait is created without
	// a title. Off leaves the truncated first message / prompt. A name the
	...
	// and untitled waits. Both empty follows the conversation's (or the
	// wait's) own model so a cheap namer is opt-in. A provider with an
	// empty model name uses that provider's configured default.
...
func Default() *Config {
	return &Config{
		...
		UI: UIConfig{
			...
			ContentFont:    DefaultContentFont,
			FontSize:       DefaultFontSize,
			CodeFont:       DefaultCodeFont,
			CodeFontSize:   DefaultCodeFontSize,
			ContentWidth:   DefaultContentWidth,
			TranscriptMode: DefaultTranscriptMode,
