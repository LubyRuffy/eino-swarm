...
hang it up). Later shells attach over loopback HTTP: they list, watch, send,
...
| `internal/lease` | exclusive `engine.lock` for one data directory (`flock`, released by the kernel on death) and `engine.json` (`pid`, `url`, `mock`, `started_at`). Discover treats a dead pid as free, a live pid that will not answer `/api/meta` as unreachable, and a mock mismatch as a different lease. The idle rule is a pure function of connected shells, the phone hub, and running turns. |
