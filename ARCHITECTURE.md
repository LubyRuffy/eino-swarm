...
One data directory has one engine process. That process holds the config, the
SQLite database, the model pool, the per-conversation runtime and the HTTP
server. Desktop, the browser and the terminal are clients of it. The first
shell starts `zwai engine` (its own session, so closing the shell does not
hang it up). A later shell of the same build attaches. A different version,
or a binary whose file identity is not the one `engine.json` published,
waits until no conversation is in a model call (a parked schedule and a
question waiting on the human do not count) and then stops that process
and starts its own. Clients attach over loopback HTTP: they list, watch, send,
steer, interrupt and answer. They do not open the database. Closing a window
or leaving the terminal only drops that client's presence connection. The
engine exits after the idle grace once no presence connection is held, the
phone hub socket is down, and no turn is running. A host that is killed
releases `engine.lock`; the next shell takes it and resumes orphaned turns.

Both graphical shells load the **same** `http://127.0.0.1` origin, so there
is exactly one implementation of uploads, downloads and the live event stream.
Traffic lights follow the window (`?shell=desktop` or the native bridge), not
the server mode string, because that mode is `engine` for every client.
...
returns `host offline`. `GET /api/remote/status` `online` follows that socket. While the pairing QR is on screen, Settings → Phone re-reads `GET /api/remote/bindings` about once a second so a new phone shows up without leaving the page; a failed read leaves the list already painted. While pairing is on
...
| `internal/clients` | read-only scan of local Claude, Codex, and Cursor transcripts for the sidebar and the phone. `View` returns the last finished snapshot and walks the disk beside the caller, so a phone inbox `list` does not wait on it. It does not start or resume those tools. |
...
| `internal/app` | wiring shared by the engine, plus listen/serve/shutdown, `openURL` and `revealPath`. `App.New` starts the schedule ticker (`Engine.StartScheduler`), the pairlink remote host, and the search embed worker; `Shutdown` stops the search worker before closing SQLite. Loopback `engine` mode still reveals files and opens URLs; a non-loopback bind does not. |
| `internal/lease` | exclusive `engine.lock` for one data directory (`flock`, released by the kernel on death) and `engine.json` (`pid`, `url`, `mock`, `started_at`, `version`, `exe_dev`, `exe_ino`). Discover treats a dead pid as free, a live pid that will not answer `/api/meta` as unreachable, and a mock mismatch as a different lease. A live engine whose `/api/meta` version or executable identity is not this binary is stopped once `GET /api/threads` shows no conversation in a model call, so the shell that just started can take the lock. The wait prints a check every 10s. After 30s a terminal can confirm a force kill (`SIGTERM`, then `SIGKILL` if the pid is still alive) and the new process takes the lock anyway. A row that is only `awaiting_answer` or `waiting` does not hold it. The idle rule is a pure function of connected shells, the phone hub, and running turns. |
...
| `internal/remote` | phone RPC over pairlink: QR offers, sealed JSON (`hello`/`list`/`more`/`open`/`start`/`send`/`steer`/`followup_drop`/`followup_steer`/`preempt`/`stop`/`answer`/`watch`/`unwatch`/`log`/`run_now`/`cancel_wait`/`resume_goal`/`catalog`/`tune`/`put`). `hello` stores the phone's model line on `remote_devices` keyed by the link fingerprint so `GET /api/remote/bindings` can show a name, not only hex. OK replies include `host` (`remote.display_name`) so the phone chip is this PC, not the hub hostname. `watch` with `since` 0 returns a live-edge snapshot (at most 24 stored events from the last turn) as the RPC `ready` `{events, more, seq, status}` (`status.waiting` / `status.wake` while a thread wait is parked); `log` `{before}` (or `before` 0 = older than that window, size `watch_events`) pages older events when the phone pulls up. Live and lagged catch-up still push `event`; goal/schedule/`done` frames include `status` so a parked `/goal` is not idle. Same kinds as desktop SSE (`PushKinds` frozen against `frontend/src/lib/stream.ts`), seq-identical, bodies clipped (`event_chars`, empty `spawned` text, 64KiB sealed frame; JSON leaves room for the seal and the host name). `list.running[].action` and `list.threads[].summary` are human one-liners (`preview.go`): findings / command / assistant prose, never a `tool_call` envelope. A parked wait has no live turn to preview, so its `action` is the thread's own summary — an In progress row that is only a title and a Waiting badge says nothing about what is being waited on. `list` drops a roster thread from `threads` so Recents does not repeat it and it does not consume `thread_limit`, which makes `running[].last_active_at` the only place a wait's age can come from. `running[].project_id` names the folder so the phone can list that same row under the project. `list.threads` is the idle recents page (`thread_limit`); live rows are `list.running` and do not consume that quota. `more` pages that idle list. A later first-page `list` patches page one and keeps rows already loaded with `more`. `catalog` lists ready models and reasoning levels with no keys or URLs. `tune` sets this conversation's provider, model, and reasoning. `put` stages a file in 36KiB chunks (256MiB cap, same as a desktop upload); an image is vision and anything else is a workspace file. Text with no attachments while a turn runs is a follow-up; attachments while a turn runs steer. Does not expose loopback `/api` to the internet. |
| `mobile/` | Capacitor iOS (`ios/`, Swift Package Manager) and Android (`android/`) apps plus the web shell. Scan QR opens a live viewfinder on a `pairlink:v1:…` QR (frame, a beam that sweeps the frame, a short chime when it reads) and is the product path; paste is the same URI. After bind, a live turn (or the last thread this phone opened, including a parked wait) opens immediately; otherwise the inbox lists In progress (five, then More), then each project and Recents with their own first page (`thread_limit`; live rows do not consume that quota). More pages that section (`list.groups`, `more.group`); a host without groups still has one idle-list More; a later first-page list patches page one and keeps rows already loaded with more. Opening a conversation that has nothing painted yet shows a loading status instead of a blank transcript. A relaunch with saved tickets paints host chips (this PC's `host` name) and a connecting skeleton; the scan form is only when nothing is saved — no ticket and no model — or after Unlink of the last PC when no model remains. Add a PC is a side sheet opened from the menu. One chrome row names which PC this is rather than the app; its corner is New chat. The inbox does not compose: search and New chat sit under the list. Each project row folds, and the icon on its right opens New chat already in that project (a project with no threads still has the row). A live conversation stays under its project as well as in In progress. New chat is its own screen (`new-chat-screen.tsx`): which PC, which project, then the message. Inbox rows are cards: a state badge (Running / Waiting / Waiting for an answer, `inbox-row.tsx`) and the row's age (`lib/when.ts`, a clock running ahead reads as now). A parked wait is In progress with **Waiting** and the thread's own summary, not a quiet Recent row. Pulling the list past the top reloads the roster (`pull-to-refresh.tsx`); an empty inbox says so. Inbox subtitles are human text (`inboxPreview`): findings, a command, or assistant prose — not `schedule_wake({…})` / `report_schedule({…})` / `memory({…})`. Opening a conversation shows the standing `/goal` (Pursuing / Done / Blocked / Paused) and, while idle, the wait banner with next check, **Run now** and **Cancel wait**. A tap paints the chrome from the listing immediately; `watch` then folds a live-edge snapshot (not the whole last turn) and jumps to the tail; **Earlier** sits above the log (pulling up also pages). User and assistant text is markdown (GFM, math). The phone defaults to the compact transcript: adjacent thoughts and tools fold into one row; an answer stays visible and splits the group. The live tail is Thinking / Planning next moves / Editing / Reading / Exec. Open the row for the pieces; a tool body stays a field preview, not packed JSON. A `progress` pulse is dropped (desktop live chrome, not a chat row). `wait_agents` is a status-count chip, not the `elapsed_ms` roster. Schedule kinds are the same one-liners as desktop (armed / fired / cancelled); `schedule_wake` / `cancel_schedule` tool rows stay off the transcript (the schedule events already notice); a findings `report_schedule` is that prose as a notice, and a quiet report is omitted. The transcript is bottom-aligned so a short thread rests on the composer, and scrolling off the tail raises **Jump to latest**. Answering inside a conversation is the composer (`composer.tsx`): a growing rounded box capped so the keyboard cannot eat the last answer, Enter sends (Shift+Enter and an IME candidate list do not), and steer sits beside send only while a turn is live. The same box picks a ready model and a thinking level (`catalog` / `tune`) and attaches files (`put`); an image is vision, anything else is a workspace file. The phone uploads, then calls `send` or `steer`; the PC decides whether that is a follow-up, a steer, or a new turn. The app talks to a `RemoteLink` (`lib/link.ts`), which is the sealed `DeviceLink` in the product and a scripted offline host under `?mock=1` (`lib/mock-link.ts`) — conversations live on that host, not on a link, because one PC down two sockets is still one PC. That is what lets Playwright reach the inbox and a conversation at all. The file browser, Settings, PTY and Trace stay on the PC. The composer can still attach a file or an image. Menu → Models saves an OpenAI-compatible endpoint on the phone (chat completions or responses). After one is saved, a Chat tab is on that same row, including when no PC is bound. That chat calls the endpoint from the phone (`openai-client.ts`). Discover uses CapacitorHttp so the webview origin is not the caller. A completion is `StreamBody` (`stream-body.ts`; iOS `URLSession` data callbacks, Android `HttpURLConnection` reads) because CapacitorHttp returns a POST body in one piece. Tokens and the thought update the same compact row as a PC thread. A responses call that sets a thinking level also asks for a summary, which is the text that API returns. A responses front that rejects an image part as having no chat completions equivalent is asked once more on chat completions. A rejected reasoning summary is omitted and the responses call is tried again. Image bytes and decoded file text stay in memory for the session and are not written next to the pairing ticket. WebSocket relay only (UDP hole-punch lives in the Go client). The ticket socket keepalives with punch-ping data frames and reconnects a drop; the inbox paints a banner instead of staying clickable-but-dead. The host-chip dot is liveness (green while the socket is up, gray while it is not, red pulse while reconnecting); relay versus direct stays on the dot's title. A failure distinguishes a socket that got no answer from a server refusal, and keeps the machine reason or the server's text. Launcher and splash are the desktop `appicon.png` mark (`go run ./mobile/scripts/genicons.go`), not Capacitor's default lattice. Hub URL is typed on the PC, never compiled into the app. Android's system back asks `window.__zwaiAndroidBack` (`mobile/src/lib/android-back.ts`, `MainActivity`): A model sheet or Add a PC pops first, then the new-conversation screen, then an open PC conversation, then a direct-model thread, and only the inbox, the chat list, or the unbound scan screen finishes the activity. The installed app checks GitHub Releases itself (`mobile/src/lib/app-update.ts`): Android downloads the `zwai-*-android.apk` and hands it to the system installer; iOS opens the release page. That automatic bar stays quiet in the browser and when the request fails. Menu → Check for updates always requests, shows that it is fetching, says when this build is current, asks before an install, and shows the response's own error. Not now only hides the automatic bar. |
| `internal/desktop` | wails3 single window pointed at the local server URL. Hidden title bar (no NSToolbar); traffic lights are centred in the 48px HTML header and the front end pads to the zoom button's measured right edge. The top 48px drags natively. A title-bar double-click is a front-end `wails:drag:doubleclick` — Wails will not zoom on the second mousedown itself, because that races the drag. The Dock / taskbar mark is an embedded PNG, inset to Apple's 824/1024 icon grid, rounded to a macOS squircle at runtime, and handed to Wails as `application.Options.Icon`, so `go run` on macOS does not keep the generic Unix-exec glyph, a square canvas, or a tile larger than a bundled `.app`. On macOS, `ReexecIfUnbundled` copies that binary into `~/Library/Caches/zwai/zwai.app` (`NSLocalNetworkUsageDescription`, stable `CFBundleIdentifier`) so Sequoia+ Local Network privacy can allow LAN model endpoints; a naked `go run` executable is identifier `a.out` and gets `no route to host` while Terminal curl works. Quit cancels the event stream so the window is not frozen waiting for it. A released Mac app reads `GET /api/update` (`internal/update`) and, after a confirm, replaces that `zwai.app` from the GitHub Release zip `zwai-<version>-darwin-<arch>.zip`. `make desktop-release` builds the zip. `make release` uploads that zip and the Android APK onto one GitHub Release. |
...
8. **Review** (project conversations with memory on, `memory.auto_review`): a
   ...
   catalog (no notes, content merge/patch/delete). `Accept: text/event-stream`
   writes the scan, the assistant prose so far, and each skill write while
   the model is still going; `done` is the same body as the JSON call. The response names what was
...
The active tick is the last send whose top has reached a 48px band under
the top of the pane. A later bubble that is only visible lower down does
not take it: at the top of a thread the next input can sit in the same
viewport, and lighting that tick disagreed with the scrollbar. The bottom
edge and a 40% line both did that. The live edge and the bottom of the
scroller still pin the latest tick. A click sticks until the reader wheels again,
