...
- 目的：在手机查看同一对话并发送、跟进、插入及翻页；使用者：手机用户；入口：手机收件箱和对话页；输入：消息与操作；输出：PC 上同一对话的事件；前置条件：已绑定且在线；失败表现：重连提示或 RPC 错误；关联契约：`C-002`, `C-003`, `C-006`；实现证据：`mobile/src/app.tsx`, `mobile/src/components/thread-screen.tsx`, `mobile/src/lib/link.ts`。
